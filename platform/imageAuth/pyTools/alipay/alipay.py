...
from pyTools.alipay.exceptions import MissingParameter, ParameterValueError, TokenAuthorizationError
...
class Alipay(object):
    ...
    def _build_url(self, service, paramnames=None, **kw):
        ''' 创建带签名的请求地址，paramnames 为需要包含的参数名，用于避免出现过多的参数，默认使用全部参数 '''
    ...
    def create_alipay_trade_precreate(self, out_trade_no, total_amount, subject):
        """ 统一收单线下交易预创建 """
        biz_content = dict()
        biz_content['out_trade_no'] = out_trade_no
        biz_content['total_amount'] = total_amount
        biz_content['subject'] = subject
        self.default_params['biz_content'] = str(biz_content)
        self.default_params['timestamp'] = get_now_time()
        self.default_params['version'] = '1.0'

        rsc = self._generate_sign(self.default_params)
...
if __name__ == '__main__':
    alipay = Alipay(key='ss', pid='ss')
    alipay.create_alipay_trade_precreate("sss", "sdsd", "rrere")
