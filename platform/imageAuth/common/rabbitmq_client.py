...
class RabbitmqClient(object):
    ...
    def rpc_call_client(self, queue_name, timeout, dict_data):
        try:
            log.info('rpc_call_client 01')
            json_data = json.dumps(dict_data)
            log.info('rpc_call_client 02')
            self.mq_connect()
            log.info('rpc_call_client 03')
            self.callback_create()
            log.info('rpc_call_client 04')
            self.rpc_call(queue_name, json_data)
            log.info('rpc_call_client 05')
            self.get_response(timeout, queue_name)
            log.info('rpc_call_client 06')
            self.mq_disconnect()
            log.info('rpc_call_client 07')

            log.info('self.response : %s' % (self.response))
