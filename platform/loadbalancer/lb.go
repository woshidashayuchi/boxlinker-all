...
const (
	...
	lbHTTPKey 		 = "serviceloadbalancer/lb.http"
	lbTCPKey                 = "serviceloadbalancer/lb.tcp"
	lbNodeKey                = "serviceloadbalancer/lb.node"
...
//  * source: connection given to backend based on hash of source ip
	...
	lbRole = flags.String("lb-role","all",`loadbalancer type http/tcp/all`)

	lbNode = flags.String("lb-node","main",`lb node main/lb1/lb2... 与 serviceloadbalancer/lb.node 关联`)
...
// backend svc_p2: pod1:tp2, pod2:tp2
	...
	kubeconfig = flags.String("kubeconfig","",`kubeconfig`)
)
...
func (newSvc service) configure(lbc *loadBalancerController, s api.Service){
...
func (s serviceAnnotations) getTcp() ([]*TcpPort, bool) {
	val, ok := s[lbTCPKey]
	if !ok {
		return nil,false
	}
	...
	for _,s := range ss {
		...
		item := &TcpPort{}
		lbPort,err := strconv.Atoi(t[0])
		if err != nil {
			continue
		}
		containerPort,err := strconv.Atoi(t[1])
		if err != nil {
			continue
		}
		item.lbPort = lbPort
...
func (s serviceAnnotations) getHttp() ([]*HttpHostPort, bool) {
	val, ok := s[lbHTTPKey]
	if !ok {
		return nil , false
	}
	...
	for _, item := range hostPorts {
		...
		hhp.port = p
		hhps = append(hhps,hhp)
	}
	return hhps, true
}

func (s serviceAnnotations) getNode() (string, bool) {
	val, ok := s[lbNodeKey]
	return val, ok
}
...
func (lbc *loadBalancerController) canHttpRole() bool {
	return lbc.role == "http" || lbc.role == "all"
}

func (lbc *loadBalancerController) canTcpRole() bool {
	return lbc.role == "tcp" || lbc.role == "all"
}
...
func (lbc *loadBalancerController) getServices() (httpSvc []service, httpsTermSvc []service, tcpSvc []service) {
	...
	for _, s := range services.Items {
		...
		for _, servicePort := range s.Spec.Ports {
			...
			httpArr,ok := annotations.getHttp()
			...
			if ok && lbc.canTcpRole() {
				for _, portMap := range portsMap {
					...
					if portMap.containerPort == sport {
						...
						if portMap.lbPort >= tcpPortMin && portMap.lbPort <= tcpPortMax {
							newSvc.FrontendPort = portMap.lbPort
							tcpSvc = append(tcpSvc, newSvc)
							glog.Infof("Found tcp service: %+v", newSvc)
						} else {
							glog.Errorf("Found service %s tcp annotation but err, lb.tcp: %v", sName,portMap)
						}
...
//func (lbc *loadBalancerController) getServices1() (httpSvc []service, httpsTermSvc []service, tcpSvc []service) {
//	ep := []string{}
//	services, _ := lbc.svcLister.List()
//	for _, s := range services.Items {
//		if s.Spec.Type == api.ServiceTypeLoadBalancer {
...
//				newSvc.Host = val
//			} else {
//				glog.Infof("Ignoring %v: no host configure", sName)
//				continue
//			}
//
//			if val, ok := serviceAnnotations(s.ObjectMeta.Annotations).getAlgorithm(); ok {
...
//			}
//
//			// todo tcp 服务的端口信息可以从 annotations 里面获取,
//			// todo 新建服务的时候需要根据用户选择的端口类型来决定是否添加这个 annotations , 端口可以随机生成但是需要唯一
//			// todo 后期当我们有多个 lb server 的时候, 还需要判断服务在哪个 lb 上面 , 再判断端口唯一
//
//			if val, ok := serviceAnnotations(s.ObjectMeta.Annotations).getTcp(); ok {
//				port, err := strconv.ParseInt(val,10,0)
//				if err == nil && port >= tcpPortMin && port <= tcpPortMax {
//					newSvc.FrontendPort = int(port)
//					tcpSvc = append(tcpSvc, newSvc)
//				} else {
//					glog.Errorf("Found service %s tcp annotation but err, lb.tcp: %s", sName,val)
//				}
//			} else {
//				if val, ok := serviceAnnotations(s.ObjectMeta.Annotations).getCookieStickySession(); ok {
//					b, err := strconv.ParseBool(val)
...
//./lb --lb-node=main --lb-role=http --use-kubernetes-cluster-service=false --server=123.56.21.188:8080 --insecure-skip-tls-verify=true
