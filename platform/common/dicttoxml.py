...
def key_is_valid_xml(key):
    ...
    LOG.debug('Inside key_is_valid_xml(). Testing "%s"' % (unicode_me(key)))
...
def make_valid_xml_name(key, attr):
    ...
    LOG.debug('Inside make_valid_xml_name(). Testing key "%s" with attr "%s"' % (
...
def convert(obj, ids, attr_type, item_func, cdata, parent='root'):
    ...
    LOG.debug('Inside convert(). obj type is: "%s", obj="%s"' % (type(obj).__name__, unicode_me(obj)))
...
def convert_dict(obj, ids, parent, attr_type, item_func, cdata):
    ...
    LOG.debug('Inside convert_dict(): obj type is: "%s", obj="%s"' % (
    ...
    for key, val in obj.items():
        LOG.debug('Looping inside convert_dict(): key="%s", val="%s", type(val)="%s"' % (
...
def convert_list(items, ids, parent, attr_type, item_func, cdata):
    ...
    for i, item in enumerate(items):
        LOG.debug('Looping inside convert_list(): item="%s", item_name="%s", type="%s"' % (
...
def convert_kv(key, val, attr_type, attr={}, cdata=False):
    ...
    LOG.debug('Inside convert_kv(): key="%s", val="%s", type(val) is: "%s"' % (
        unicode_me(key), unicode_me(val), type(val).__name__)
    )
...
def convert_bool(key, val, attr_type, attr={}, cdata=False):
    ...
    LOG.debug('Inside convert_bool(): key="%s", val="%s", type(val) is: "%s"' % (
        unicode_me(key), unicode_me(val), type(val).__name__)
    )
...
def convert_none(key, val, attr_type, attr={}, cdata=False):
    ...
    LOG.debug('Inside convert_none(): key="%s"' % (unicode_me(key)))
...
def dicttoxml(obj, root=True, custom_root='root', ids=False, attr_type=True,
    ...
    - cdata specifies whether string values should be wrapped in CDATA sections.
      Default is False
    """
    LOG.debug('Inside dicttoxml(): type(obj) is: "%s", obj="%s"' % (type(obj).__name__, unicode_me(obj)))
    output = []
    addline = output.append
    if root == True:
